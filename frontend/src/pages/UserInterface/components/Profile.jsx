...
function Profile({ userData }) {
   const strategyIcons = {
    ...
    // Generate avatar using email
    const avatar = userData.email 
      ? `https://api.dicebear.com/7.x/avataaars/svg?seed=${userData.email}` 
      : 'https://api.dicebear.com/7.x/avataaars/svg?seed=fallback';
      
    // Use strategy_type from the response
    const strategy = userData.strategy_type || 'Branding Strategy';
  return (
        ...
        <div className="bg-white/10 backdrop-blur-md p-6 rounded-lg flex-1">
          ...
          <div className="space-y-4 text-white">
            ...
            <div className="flex space-x-2">
              ...
              <p>{userData.goal || 'Not specified'}</p>
            </div>
            <div className="flex space-x-2">
              <p className="text-white font-semibold">Description:</p>
              <p>{userData.comment || 'Not available'}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white/10 backdrop-blur-md p-6 rounded-lg flex-1">
          ...
          <div className="space-y-4">
            <div className="bg-white/5 p-4 rounded">
              ...
              <p className="text-white">{userData.subscription?.plan || 'Basic Plan'}</p>
            </div>
            <div className="bg-white/5 p-4 rounded">
            ...
            <div className="bg-white/5 p-4 rounded">
              ...
              <p className="text-white">{userData.created_at || 'Not available'}</p>
            </div>
            <div className="bg-white/5 p-4 rounded">
              <p className="text-[#21BFE4] font-semibold">Subscription End Date</p>
              <p className="text-white">{userData.subscription?.end_date || 'Not available'}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mt-8">
        ...
        <div className="bg-white/10 backdrop-blur-md p-6 rounded-lg">
          <div className="flex items-center gap-4 mb-4 text-white">
            {strategyIcons[strategy]?.platformIcon || <FaInstagram className="text-2xl" />}
            <h3 className="text-xl font-bold text-white">{strategy}</h3>
          </div>
          
          <p className="text-gray-300 mb-6">
            {strategyDescriptions[strategy] || 'Custom strategy for content creation and audience growth.'}
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-white/5 p-4 rounded">
              ...
              <div className="flex gap-2 mt-2 text-white">
                {strategyIcons[strategy]?.platform || 'Instagram'}
              </div>
            </div>
            ...
            <div className="bg-white/5 p-4 rounded">
              ...
              <div className="flex items-center mt-2">
                <span className={`px-2 py-[0.25em] text-sm font-semibold rounded-full bg-white ${userStatus[userData.status || 'active']}`}>
