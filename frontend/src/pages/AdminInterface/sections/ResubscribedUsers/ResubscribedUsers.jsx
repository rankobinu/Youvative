...
function ResubscribedUsers() {
  ...
  useEffect(() => {
    const fetchResubscribedUsers = async () => {
      try {
        setIsLoading(true);
        const response = await adminService.getResubscribedUsers();
        ...
        setError(error.message || 'Failed to load resubscribed users');
      } finally {
        setIsLoading(false);
      }
    };

    fetchResubscribedUsers();
  }, []);
        ...
        <div className="space-y-6 p-4">
          ...
          {users.length === 0 ? (
            <div className="bg-white/10 backdrop-blur-md p-6 rounded-lg text-center">
              <p className="text-white text-lg">No resubscribed users found.</p>
            </div>
          ) : (
                ...
                >
                  ...
                  <div className="pt-12 p-6 text-center">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className='px-3 py-[0.25em] text-sm font-semibold rounded-full bg-white text-yellow-600'>
                        {user.status}
                      </span>
