...
function NewUsers() {
  ...
  useEffect(() => {
    const fetchNewUsers = async () => {
      try {
        setIsLoading(true);
        const response = await adminService.getNewUsers();
        ...
        setError(error.message || "Failed to load new users");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSetStrategy = async (userId) => {
    try {
      // First fetch the user details
      await adminService.getUserDetails(userId);

      // Then navigate to the strategy page
    ...
    <>
      {isLoading ? (
        ...
        <div className="space-y-6 p-4">
          <h1 className="text-3xl font-bold text-white mb-8">New Users</h1>
          ...
          {users.length === 0 ? (
            <div className="bg-white/10 backdrop-blur-md p-6 rounded-lg text-center">
              <p className="text-white text-lg">No new users found.</p>
            </div>
          ) : (
                ...
                >
                  ...
                  <div className="pt-12 p-6 text-center">
                    ...
                    <div className="flex justify-between items-center">
                      <span className="px-3 py-[0.25em] text-sm font-semibold rounded-full bg-white text-blue-600">
                        {user.status}
                      </span>
