...
function NewUserStrategy() {
  ...
  const handleSubmitStrategy = async () => {
    ...
    if (!monthlyGoal || !monthlyDescription || tasks.length === 0) {
      alert('Please set a monthly goal, description, and add at least one task');
      return;
    }
    ...
    try {
      ...
      // Send to API
      const response = await adminService.setUserStrategy(userId, strategyData);
            ...
            <div className="mt-8 space-y-8">
              ...
              <div className="bg-white/10 backdrop-blur-md p-6 rounded-lg">
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    <label className="block text-white mb-2">Monthly Strategy Description</label>
                    <textarea
              ...
              {strategySubmitted && (
                <div className="mt-2 p-3 bg-green-500/20 border border-green-500 rounded-lg text-green-400 flex items-center">
                  <FiCheckCircle className="mr-2" /> Strategy has been set successfully!
                </div>
              )}
            </div>
          )}
