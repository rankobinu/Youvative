...
function StrategySection() {
  ...
  const { status, error: registrationError } = useSelector(
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      setVisibleRows(
        Math.min(newVisibleRows, Math.ceil(strategies.length / 4)),
  ...
  const strategies = [
    ...
    },
    {
      ...
      description:
        "Focus on dominating a specific niche (e.g., fitness, tech, etc.)",
      platform: [
        <FaTiktok key="tiktok" />,
  ...
  return (
    <div className="flex flex-col  bg-gradient-to-t from-[#5E4683] to-[#150F1D]">
      {/* Fixed Header */}
      <div
        className={`sticky flex top-0 right-0 left-0 -px-8 ${showTransition ? "transition-color" : ""}  ${scrolled ? "bg-[#150F1D]" : ""} my-5 py-3 z-10`}
      >
        <div>
          <RouterLink to="/homepage">
            <img
              src={logo}
              alt="Youvative Logo"
              className="absolute -my-10 -ml-10 "
            />
          </RouterLink>
        </div>
        <div className="text-center mx-auto">
          ...
          <p className="text-white/80 mt-2 text-lg">
            Select the strategy that best aligns with your content creation
            goals
          </p>
        </div>
      </div>
